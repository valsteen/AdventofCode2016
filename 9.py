...
def advent9(seq):
    ...
    while seq:
        matches = re.match("(?P<asis>.*?)(\((?P<len>[0-9]+)x(?P<times>[0-9]+)\))(?P<rest>.*)", seq)
        if not matches:
            return res + seq
        asis = matches.group("asis")
        length = matches.group("len")
        times = matches.group("times")
...
class Multiplier(object):
    ...
    def __len__(self):
        return len(str(self.size)) + len(str(self.times)) + 3

    def __str__(self):
        return "({}x{})".format(self.size, self.times)

    def __repr__(self):
        return str(self)


class DecompressedString(object):
...
def advent9part2(seq):
    ...
    while seq:
        ...
        if not matches:
            tokenized.append(DecompressedString(len(seq)))
            break
        ...
        if asis:
            tokenized.append(DecompressedString(len(asis)))

        if length:
            assert times

            multiplier = Multiplier(int(length), int(times))
            tokenized.append(multiplier)

        seq = rest

    return compute_length(tokenized)
...
def compute_length(seq):
    ...
    while seq:
        item = seq.pop(0)
        if isinstance(item, DecompressedString):
            length += len(item)
        elif isinstance(item, Multiplier):
            targets = []
            while sum(len(i) for i in targets) < item.size:
                obj = seq.pop(0)
                targets.append(obj)
            targets_size = sum(len(i) for i in targets)
            if targets_size > item.size:
                assert isinstance(targets[-1], DecompressedString)
                targets[-1] = DecompressedString(len(targets[-1]) - (targets_size - item.size))
                length += targets_size - item.size
            length += item.times * compute_length(targets)
        else:
